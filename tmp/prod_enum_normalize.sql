...
SET slot = CASE
    ...
    WHEN lower(btrim(slot::text)) IN ('legs') THEN 'LEGS'
    WHEN lower(btrim(slot::text)) IN ('boots', 'feet') THEN 'BOOTS'
    WHEN lower(btrim(slot::text)) IN ('pocket') THEN 'POCKET'
...
SELECT 'equipment.class' AS column_name, "class" AS value, count(*)
FROM equipment
WHERE "class" IS NOT NULL
GROUP BY "class"
ORDER BY "class";

SELECT 'equipment.slot' AS column_name, slot AS value, count(*)
FROM equipment
WHERE slot IS NOT NULL
GROUP BY slot
ORDER BY slot;

SELECT 'equipment.style' AS column_name, style AS value, count(*)
FROM equipment
GROUP BY style
ORDER BY style;

SELECT 'equipment.type' AS column_name, type AS value, count(*)
FROM equipment
GROUP BY type
ORDER BY type;

SELECT 'targets.primarystyle' AS column_name, primarystyle AS value, count(*)
FROM targets
WHERE primarystyle IS NOT NULL
GROUP BY primarystyle
ORDER BY primarystyle;
