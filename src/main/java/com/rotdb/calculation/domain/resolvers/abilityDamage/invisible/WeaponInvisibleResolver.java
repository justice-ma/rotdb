...
public class WeaponInvisibleResolver {
    public static double resolve(CalculationContext context) {
        ...
        if (mainhand.getEffect().contains(Effect.HEX)) {
            if (style == CombatStyles.MELEE && buff.has(BuffId.ENCHANTMENTOFSAVAGERY) ||
                    style == CombatStyles.MAGIC && buff.has(BuffId.ENCHANTMENTOFAFFLICTION) ||
                    style == CombatStyles.RANGED && buff.has(BuffId.ENCHANTMENTOFDISPELLING)) {
                return 1.175;
            } else {
                return 1.125;
            }
